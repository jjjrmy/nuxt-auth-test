  ...
  <header class="flex flex-wrap sm:justify-start sm:flex-nowrap z-50 w-full bg-blue-600 text-sm py-3 sm:py-0">
    <nav class="relative max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8" aria-label="Global">
      <div class="flex items-center justify-between">
        <a class="flex-none text-xl font-semibold text-white" href="#" aria-label="Nuxt-Laravel-Auth">Nuxt-Laravel-Auth</a>
        <div class="sm:hidden">
      ...
      <div id="navbar-collapse-with-animation" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow sm:block">
        <div class="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:ps-7">
          <NuxtLink class="font-medium text-white/[.8] hover:text-white sm:py-6" activeClass="!text-white" to="/">Homepage</NuxtLink>
          <NuxtLink class="font-medium text-white/[.8] hover:text-white sm:py-6" activeClass="!text-white" to="/pricing">Pricing</NuxtLink>
          <NuxtLink class="font-medium text-white/[.8] hover:text-white sm:py-6" activeClass="!text-white" to="/dashboard">Dashboard</NuxtLink>
          <template v-if="!isAuthenticated">
            <NuxtLink class="flex items-center gap-x-2 font-medium text-white/[.8] hover:text-white sm:border-s sm:border-white/[.3] sm:my-6 sm:ps-6" activeClass="!text-white" to="/login">
              <Icon name="ri:user-3-line" class="flex-shrink-0 w-4 h-4" />
              <span>Log in</span>
            </NuxtLink>
          </template>
          <template v-else>
            <NuxtLink class="flex items-center gap-x-2 font-medium text-white/[.8] hover:text-white sm:border-s sm:border-white/[.3] sm:my-6 sm:ps-6" @click="logout">
              <Icon name="ri:logout-circle-line" class="flex-shrink-0 w-4 h-4" />
              <span>Log out</span>
            </NuxtLink>
          </template>
        </div>
